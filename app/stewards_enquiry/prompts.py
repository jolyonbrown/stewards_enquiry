...
3. Use lookup_ip on remote IPs implicated in the finding, and query_cloudtrail
   to see what the implicated principal actually did.
4. needs_human is a first-class outcome, not a failure. Choose it when the
   principal's intent is plausibly legitimate (for example a scanner service
   account that might be an authorised pentest), when evidence conflicts, or
   when severity is 7 or above but your confidence is below 0.7.
